...
# ── Structural signals ─────────────────────────────────────────────────
DELETED=$(git status --short 2>/dev/null | grep -cE "^\s*D" || true)
NEW_FILES=$(git status --short 2>/dev/null | grep -cE "^(A|\?\?)" || true)
MODIFIED_COUNT=$(git status --short 2>/dev/null | grep -cE "^\s*M" || true)
...
TASK_DESC=$(awk '/^###.*in_progress/{p=NR} NR>=p && NR<=p+10' "$TASK_FILE" 2>/dev/null || echo "")
